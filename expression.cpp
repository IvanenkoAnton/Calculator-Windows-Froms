...
std::string Calculate(const char *string) {
			...
			if (isOperation(buffer[0]) || isFunction(buffer)) {
				if (buffer != "(") {
					...
					if (Oper->isEmpty()) {;
						...
						if (*op > Oper->GetTopData()) { //высшый приоритет
							Oper->push_front(*(new Operation(buffer, ReturnPriority(buffer, isUnarOperation))));
							isNumber = true;
							isOper = false;
						}
						else {
							if (buffer == ")") { // если встретили закрывающую скобку
								Operation operation = Oper->pop_front();
								std::string oper = operation.oper;
								int prior = operation.priority;
								while (oper != "(") {
									if (prior == 5) { //если унарная операция
										op2 = 0;
							...
							else { //если операция имеет меньший или такой же приоритет
								Operation operation = Oper->pop_front();
								std::string oper = operation.oper;
								int prior = operation.priority;
								if (prior == 5) {
	...
	while (!Num->isEmpty() && !Oper->isEmpty()) {//вычисление оставшихся операций в примере
		Operation operation;
		std::string oper;
		int prior;
		if (!Oper->isEmpty()) {
